...
| **Comparador vs Média do Segmento** *(PRD 04, schema 1.7)* | Snapshot do box "Média Tipo/Segmento" do I10 (P/VP médio, DY 12M médio, Valor Patrimonial total, VPA médio + grupo SEGMENTO/TIPO). Cálculos puros em `src/shared/peer.js`: desvio percentual por métrica, classificação `FAVORAVEL`/`NEUTRO`/`DESFAVORAVEL`/`SEM_DADOS` (precedência do DESFAVORAVEL), `preco_referencia_peer`, **`preco_teto_efetivo` (peer nunca eleva o teto base)**, multiplicador de rebalanceamento. **Simulação de aporte** em `POST /api/dashboard/rebalanceamento` com distribuição proporcional ao peso (`gap_alvo × multiplicador_peer`) e redistribuição de sobra. Endpoints `GET /api/fiis/:ticker/comparativo-peer` (400 ticker inválido, 404 não-FII, estado `SEM_DADOS`) e `POST /api/dashboard/rebalanceamento` (400 aporte inválido). Helpers de UI puros em `src/shared/peer-ui.js` e `src/shared/rebalanceamento-ui.js` prontos para integração visual |
