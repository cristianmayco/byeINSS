...
- **Validação rigorosa de inputs em todas as rotas.** Os endpoints do PRD 12 (vencimento de contratos), PRD 02 (indicadores históricos), **PRD 03 (amortizações em proventos)**, **PRD 01 (histórico de dividendos)** e **PRD 04 (comparador vs média do segmento)** aplicam regex estrito no `:ticker` (`^[A-Z]{4}11$` para as rotas legadas; `^[A-Z]{4}\d{1,2}$` para `/api/fii-historico/:ticker` aceita 1-2 dígitos), allowlist de tipos em `CHECK` constraint (`DIVIDENDO | RENDIMENTO | AMORTIZACAO | BONIFICACAO`), allowlist de campos mutáveis no `PUT`, prepared statements em todos os SQL e mensagens de erro genéricas (não vazam SQL paths). O **PRD 03** adiciona deduplicação por chave lógica completa (RF-007), reconciliação opcional de legados (RF-008) e parsing do tipo por HEADER semântico (RF-004) — tipos desconhecidos NÃO viram `DIVIDENDO` silenciosamente. O **PRD 01** adiciona dedup por `(fonte, origem_chave)`, reconciliação com manuais (manuais NUNCA sobrescritos) e parser de histórico por HEADER semântico. O **PRD 04** adiciona validação estrita de aporte numérico finito > 0 em `POST /api/dashboard/rebalanceamento` (rejeita NaN, negativo, string), `mergeSnapshotPeer` com whitelist que ignora propriedades injetadas, snapshot atômico do benchmark do I10 (RF-005: snapshot só é válido se os 4 numéricos forem extraídos juntos) e `preco_teto_efetivo` que **nunca eleva** o teto base (RF-016). Ver `src/server/routes/proventos.js`, `src/server/routes/dashboard.js`, `src/server/routes/fii-historico.js`, `src/server/routes/peer.js`, `src/shared/proventos-import.js`, `src/shared/proventos-helpers.js`, `src/shared/dividendos-hist.js`, `src/shared/agenda-parser.js`, `src/shared/dividendos-import.js`, `src/shared/peer.js`, `src/shared/scraper-peer.js`.
