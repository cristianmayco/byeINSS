...
- **Validação rigorosa de inputs em todas as rotas.** Os endpoints do PRD 12 (vencimento de contratos) e do PRD 02 (indicadores históricos) aplicam regex estrito no `:ticker` (`^[A-Z]{4}11$`), allowlist de campos mutáveis no `PUT`, prepared statements em todos os SQL e mensagens de erro genéricas (não vazam SQL paths). Ver `src/server/routes/contratos.js`, `src/server/routes/indicadores.js`, `src/server/routes/scraper-indicadores.js` + `src/shared/contratos.js` + `src/shared/indicadores.js`.
