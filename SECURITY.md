...
- **Validação rigorosa de inputs em todas as rotas.** Os endpoints do PRD 12 (vencimento de contratos), PRD 02 (indicadores históricos) e **PRD 03 (amortizações em proventos)** aplicam regex estrito no `:ticker` (`^[A-Z]{4}11$`), allowlist de tipos em `CHECK` constraint (`DIVIDENDO | RENDIMENTO | AMORTIZACAO | BONIFICACAO`), allowlist de campos mutáveis no `PUT`, prepared statements em todos os SQL e mensagens de erro genéricas (não vazam SQL paths). O **PRD 03** adiciona deduplicação por chave lógica completa (RF-007), reconciliação opcional de legados (RF-008) e parsing do tipo por HEADER semântico (RF-004) — tipos desconhecidos NÃO viram `DIVIDENDO` silenciosamente. Ver `src/server/routes/proventos.js`, `src/server/routes/dashboard.js`, `src/shared/proventos-import.js`, `src/shared/proventos-helpers.js` + `src/shared/agenda-parser.js`.
